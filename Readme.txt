...
1. はじめに
  Biscuit(MAP)は Controller-View-Model もどきのミニ・フレームワークです。
  どちらかというとフレームワークというより、MVCクラスライブラリと言った方が
  的確かもしれません。
  特にビューテンプレートが特徴的で、一般的なフレームワークと異なり独自の
  「セクションテンプレート」と呼ぶ形式を採用しています。
  詳しくは機能詳細をご覧いただくとして、レイアウトのデザインが驚くほど簡単に
  できてしまいます。
  フレームワーク内にはそのセクションテンプレートを全力でサポートする機能が
  盛り込まれています。
  ...
  特にViewの中から動的にデータを取得し表示整形できるように Model オブジェクトを
  呼び出せるようにしていて、コントローラ経由のデータ渡しや、セッション変数に記憶
  する方式にくらべ、大量のデータレコードを扱う時でもメモリ消費を少なくすることができます。
  ただし、その代償として View と Model の分離が曖昧になるというデメリットが発生しますが、
  View における大量データ処理でもメモリ消費を抑え、柔軟な処理コードが実現できる
  というメリットを優先しました。

  Viewテンプレートは基本的にPHP/HTML記述ですが、連想配列を利用してレイアウトを簡単に
  組み立てられる「セクションレイアウト」方式を実装しています。
  セクションレイアウト形式はCSSやJavascript、多言語リソースファイルでも利用している
  共通テンプレート形式です。

  Biscuit では「モジュールMVC」という考え方に基づいて、アプリケーション開発を
  モジュール単位で行えるようにしています。
  これにより、新しいモジュールの開発・デバッグが既存モジュールに影響を与えず
  実装できるようにしています。
...
(vendor)            ベンダーライブラリフォルダ
  ...
  ├ vendor                     composerでインストールするライブラリ
  │  └─ mpdf70               PDF作成ライブラリ
  │
  ├ Twig                      TWIGライブラリ
  └ FileMaker                 FileMaker PHP-API

  一般的なフレームワークではコントローラ(Controller)、モデル(Model)、ビュー(View)の
  各フォルダに実装ファイルを格納し、フォルダごとに担当者を分担する方式が主流です。
  大人数で役割分担された体制では効果的ですが、単独開発や少人数の体制では機能ファイルが各フォルダに
  分散してしまうため、ファイル間の見通しが悪くなり修正漏れなどが起きやすいという欠点があります。
  実際に作者は、とあるフレームワークで Helper ファイルの存在を忘れ修正漏れに気づかず
  デバッグに苦労した経験があります。

  そこで本フレームワークではMVCの各ファイルを「モジュール」という形でまとめることで
  関係するファイルをひとつのフォルダに集約し管理しやすくしています。
  「モジュール」というフォルダ単位で管理することで、次のようなメリットがあります。
    ・他のモジュールの影響を受けず、モジュール単位で独立した開発が行える
    ・クラス継承の階層とフォルダ階層がほぼ同じ構成になるのでファイルの見通しが良い

  本フレームワークにはコードの自動生成ツール(CakePHPのBake等)が含まれていないので、
  データベースの定義はデータベース用のツール(pgAdmin等)を使用します。
  またベースコードの実装も、全てスクラッチからコーディングしてください。
...
1.3 フレームワーク・クラス
  ...
  Biscuit は大変シンプルなクラスオブジェクト群で構成しています。
  MVCに相当する基底クラスに、ViewにおけるHTMLヘルパーのみで構成しています。
...
■ クラス相関図
  ...
  ◇ フレームワーク拡張ステップ
    ...
    ・拡張コアクラスが他のアプリケーションでも再利用可能なメソッドであればフレームワークのコアクラスに再定義する
...
　コアクラスの AppController を継承することで、モジュール側の実装は最小限で済みます。
　またコアクラスのアクションメソッドを無効化したり、デフォルトのアクションを変更することができます。

　コアクラスの AppController では、所属するモジュールのモデルクラスおよびヘルパークラスを生成します。
...
■ 画面レイアウト定義
  ...
  フレームワーク変数は以下のものがあります。
    ...
    {$URIROOT$}     フレームワークのトップフォルダ
    {$WEBROOT$}     app/webroot/ へのパス
    {$URI$}         リクエストされたURI
    {$REFERER$}  リファラー
    {$QUERY$}       ? 以降のクエリー文字列
    {$CONTROLLER$}  トップフォルダから2番目までのURIパス
    {$METHOD$}      トップフォルダから3番目までのURIパス
    {$FILTER$}      トップフォルダから4番目までのURIパス
    {$PARAMS$}      トップフォルダから5番目以降のURIパス
    {$controller$}  コントローラ名
    {$method$}      'Action'文字列を除いたメソッド名
    {$filter$}      フィルタ名
    {%0%}～{%9%}    フィルタ以降の数値パラメータ
...
■ 画面レイアウト テンプレート (*.tpl)
 Section = {                              *** タグ・セクションの定義 ***
  @XXXXX  => [ key => Value ]             テンプレート呼び出し、配列は変数として引き渡し、連想配列にしなければ引数なしの呼び出し
                                          XXXX がドット(.)で始まる名前はインラインセクション名を参照し、それ以外は外部ファイルを参照する
  ...
  *Comment                                コメントタグの出力
  ...
  .tpl はViewの中で配列に再構築してから、セクション処理によりタグ出力する
  セクションアイテムの value 部分にはてくださいた場合、モジュール名リソースからの相対参照となります。
  モジュールリソース内に識別名が無い場合やドットで始まらない識別名は絶対参照でリソース検索します。
...
■ 言語リソース テンプレート (*.lng)
  言語リソースは「言語名(ja)＋拡張子(lng)」で言語フォルダへ格納します。
...
・静的変数
  ...
  App::$argv;                   // メソッド以下のURIを/で分割した文字列
  App::$argc;                   // 引数の数
  App::$ActionClass;            // コントローラ名
  App::$ActionMethod;           // メソッド名
  App::$RunTime;                // コントローラを呼び出した時間
...
・静的メソッド
  ...
  APPDEBUG::debug_dump(出力レベル,出力配列, メッセージレベル)   // デバッグ情報ダンプ
  APPDEBUG::arraydump(メッセージレベル,出力配列)                // 配列のダンプ
...
◆ MySession クラス
・静的変数
  MySession::$PostEnv;                  // REQUEST されてきた変数(POST/GET)
...
・静的メソッド
  ...
  MySession::SetVars($nm,$val)          // POST変数に値をセット
...
・ヘルパークラス(AppHelper)
      getParams($n, $alt)                 // パラメータ指定の有無を判定し、存在しなければ$altを返す
...
3.0 コントロール・ライブラリ
  ...
  Biscuitはアプリケーションの構築に役立つプリミティブなコントロールコンポーネントが
